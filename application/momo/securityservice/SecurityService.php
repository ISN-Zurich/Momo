...
namespace momo\securityservice;
 ...
 * @package momo.application.managers.securityservice
 */
class SecurityService extends BaseService {
	...
	public function authenticate($login, $password) {
		...
		try {
			...
			if ( 	   ( $user !== null )
				...
				$this->getCI()->session->set_userdata('securityservice.user_login', $user->getLogin());	
			...
			throw new MomoException("SecurityService:authenticate() - a database error occurred", 0 , $ex);
		}
		
		// authenticate and store user object in session
		$this->getCI()->session->set_userdata('securityservice.user_authenticated', $authenticated);
	...
	public function authorize($permission) {
		...
		if ($this->getCI()->session->userdata('securityservice.user_authenticated') !== null) {
