<?php



/**
 * This class defines the structure of the 'projects' table.
 *
 *
 *
 * This map class is used by Propel to do runtime db structure discovery.
 * For example, the createSelectSql() method checks the type of a given column used in an
 * ORDER BY clause to know whether it needs to apply SQL to make the ORDER BY case-insensitive
 * (i.e. if it's a text column type).
 *
 * @package    propel.generator.momo.map
 */
class ProjectTableMap extends TableMap
{

	/**
	 * The (dot-path) name of this class
	 */
	const CLASS_NAME = 'momo.map.ProjectTableMap';

	/**
	 * Initialize the table attributes, columns and validators
	 * Relations are not initialized by this method since they are lazy loaded
	 *
	 * @return     void
	 * @throws     PropelException
	 */
	public function initialize()
	{
		// attributes
		$this->setName('projects');
		$this->setPhpName('Project');
		$this->setClassname('Project');
		$this->setPackage('momo');
		$this->setUseIdGenerator(true);
		// columns
		$this->addPrimaryKey('ID', 'Id', 'INTEGER', true, null, null);
		$this->addColumn('NAME', 'Name', 'VARCHAR', true, 50, null);
		$this->addColumn('ENABLED', 'Enabled', 'BOOLEAN', true, 1, true);
		$this->addColumn('ARCHIVED', 'Archived', 'BOOLEAN', true, 1, false);
		// validators
	} // initialize()

	/**
	 * Build the RelationMap objects for this table relationships
	 */
	public function buildRelations()
	{
		$this->addRelation('TeamProject', 'TeamProject', RelationMap::ONE_TO_MANY, array('id' => 'project_id', ), null, null, 'TeamProjects');
		$this->addRelation('UserProject', 'UserProject', RelationMap::ONE_TO_MANY, array('id' => 'project_id', ), null, null, 'UserProjects');
		$this->addRelation('ProjectEntry', 'ProjectEntry', RelationMap::ONE_TO_MANY, array('id' => 'project_id', ), 'CASCADE', null, 'ProjectEntrys');
		$this->addRelation('Team', 'Team', RelationMap::MANY_TO_MANY, array(), null, null, 'Teams');
		$this->addRelation('User', 'User', RelationMap::MANY_TO_MANY, array(), null, null, 'Users');
	} // buildRelations()

} // ProjectTableMap
