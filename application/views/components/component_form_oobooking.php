...
$component_mode == "new" ? $action = "/managebookings/createbooking" : $action = "/managebookings/updatebooking";
...
<script type="text/javascript">
	...
	//
	function cancelForm() {
		document.location = "/managebookings";
	}


	//
	// validates the oo booking form
	//
	...
	// - "fromDate" <= "untilDate"
	//
	// if the type to validate is "user generated" and "paid" - we also
	// need to validate that a selection for the "auto worktime credit"
	...
	function isValidForm() {
		...
		var typeIsPaidData		= $("select[name=typeId] option:selected").data("paid");
		var typeCreatorData		= $("select[name=typeId] option:selected").data("creator");
		...
		var autoAssignWorktimeCreditValue	= $(form).find("select[name=autoAssignWorktimeCredit]").val();
		...
		// validate that the indicated date range does not conflict with an existing oo booking or regularentry (ajax call)
		...
		// validate that "paid" user types indicate whether to auto assign worktime credit
		else if ( typeIsPaidData && (typeCreatorData == '<?php echo \OOBookingType::CREATOR_USER?>') ) {
	...
	function hasBookingConflict(userId, fromDate, untilDate) {
		...
		?>
			...
			ajaxUrl += "/" + <?php echo $component_edit_target->getId(); ?>
	...
	$(function() {
		...
		$("#fromDate .datePicker").datepicker({
			...
			minDate: new Date(	<?php echo $component_date_elems_earliest_booking["year"]; ?>,
								<?php echo ($component_date_elems_earliest_booking["month"] - 1); ?>,
								<?php echo $component_date_elems_earliest_booking["day"]; ?>),
								
			maxDate:  new Date(	<?php echo $component_date_elems_latest_booking["year"]; ?>,
								<?php echo ($component_date_elems_latest_booking["month"] - 1); ?>,
								<?php echo $component_date_elems_latest_booking["day"]; ?>)
		});
		...
		$("#untilDate .datePicker").datepicker({
			...
			<?php else : ?>
				...
				minDate: new Date(	<?php echo DateTimeHelper::getYearFromDateTime($component_edit_target->getStartDate()); ?>,
									<?php echo (DateTimeHelper::getMonthFromDateTime($component_edit_target->getStartDate()) - 1); ?>,
									<?php echo DateTimeHelper::getDayFromDateTime($component_edit_target->getStartDate()); ?>),
		...
		$("#halfDaysAM .datePicker").datepicker({
			...
			<?php if ( $component_mode == "edit" ) : ?>
				...
				,minDate: new Date(	<?php echo DateTimeHelper::getYearFromDateTime($component_edit_target->getStartDate()); ?>,
				 ...
				 maxDate: new Date(	<?php echo DateTimeHelper::getYearFromDateTime($component_edit_target->getEndDate()); ?>,
									<?php echo (DateTimeHelper::getMonthFromDateTime($component_edit_target->getEndDate()) - 1); ?>,
									<?php echo DateTimeHelper::getDayFromDateTime($component_edit_target->getEndDate()); ?>)
		...
		$("#typeId").change(function() {
			...
			// if a type is selected, populate "book by" and "auto assign worktime credit" according to type
			if ( $("#typeId").val() != -1 ) {
				...
				var typeIsPaid				 	= $("#typeId option:selected").data("paid");
				var typeCreator					= $("#typeId option:selected").data("creator");

				//
				// display booking range controls according to type
				if ( typeIsBookableInDays && ( ! typeIsBookableInHalfDays) ) {

					// make both "half-day" and "full day" controls visible
					...
					// make only "full day" controls visible
					$('#book_by_day_controls').css("display", "block");
					// hide half day controls
					$('#book_by_halfday_controls').css("display", "block");	
				}

				//
				// finall, make container for entry type controls visible
				$('#secondary_controls').css("display", "block");

				//
				// display "auto worktime credit" control according to type
				// -- auto assignment of worktime credit is only an option, if we're dealing with a paid and "user generated" type
				if ( typeIsPaid && (typeCreator == '<?php echo \OOBookingType::CREATOR_USER?>') ) {
					// make "auto worktime credit" controls visible
					$('#autoAssignWorktimeCredit_controls').css("display", "block");
				}
				else {
					$('#autoAssignWorktimeCredit_controls').css("display", "none");
				}
					...
					<fieldset>
								...
								<select id="typeId" name="typeId" class="span3" <?php echo $disabled; ?>>
									...
									<?php foreach ( $component_bookingtypes as $curType ) : ?>
										...
										<?php if ( $component_mode == "edit" ) : ?>
											...
											<?php if ( $component_edit_target->getOOBookingType() == $curType ) : ?>
										...
										<option data-bookableindays="<?php print( $curType->getBookableInDays() ? "true" : "false"); ?>"
												...
												data-paid="<?php print( $curType->getPaid() ? "true" : "false"); ?>"
												data-creator="<?php print( $curType->getCreator() ); ?>"
						...
						<?php
							...
							if ( $component_mode == "edit" ) {
								...
								if ( $component_edit_target->getOOBookingType()->getBookableInDays() ) {
									$bookByDayControlsStyle = "display: block";
								}
								
								if ( $component_edit_target->getOOBookingType()->getBookableInHalfDays() ) {
						...
						<div id="secondary_controls" style="<?php echo $secondaryControlsStyle; ?>">
							...
							<div id="book_by_day_controls" style="<?php echo $bookByDayControlsStyle; ?>">
								...
								<div class="control-group">
									...
									<div class="controls">
										<div id="fromDate">
											<?php $component_mode == "edit" ? $fromDate = DateTimeHelper::formatDateTimeToStandardDateFormat($component_edit_target->getStartDate()) : $fromDate = "" ?>
								...
								<div class="control-group">
									...
									<div class="controls">
										<div id="untilDate">
											<?php $component_mode == "edit" ? $untilDate = DateTimeHelper::formatDateTimeToStandardDateFormat($component_edit_target->getEndDate()) : $untilDate = "" ?>
											...
											<div class="controls">
												<select multiple="multiple" name="halfDaysAM[]" class="span2 uneditable-input">
													...
													<?php if ( $component_mode == "edit" ) : ?>
															...
															<?php foreach ( $component_edit_target->getHalfDayMarkerEntriesAM() as $curEntry ) : ?>
											...
											<div class="controls">
												<select multiple="multiple" name="halfDaysPM[]" class="span2 uneditable-input">
													...
													<?php if ( $component_mode == "edit" ) : ?>
														...
														<?php foreach ( $component_edit_target->getHalfDayMarkerEntriesPM() as $curEntry ) : ?>
								...
								<?php 	
									// if the booking is paid, and we're not working with a system generated booking type
									// users may indicate whether to automatically assign worktime credit
									$autoAssignWorktimeCreditControlsStyle = "display: none;";
									
									if ( 		($component_mode == "edit")
											&& 	($component_edit_target->getOOBookingType()->getPaid())
											&& 	($component_edit_target->getOOBookingType()->getCreator() == \OOBookingType::CREATOR_USER)
										) {
										
										$autoAssignWorktimeCreditControlsStyle = "display: block;";
									}
								?>
											...
											<select id="autoAssignWorktimeCredit" name="autoAssignWorktimeCredit" class="span1">
												...
												<?php foreach (\momo\core\helpers\FormHelper::$binaryOptionsYesNo as $curOptVal => $curOptText) : ?>
													...
													<?php if ( $component_mode == "edit" ) : ?>
														...
														<?php if ( $component_edit_target->getAutoAssignWorktimeCredit() == $curOptVal ) : ?>
							...
							<?php 	
								// if the booking is associated with a request, we display request related info/controls
								if ( 		($component_mode == "edit")
										&&  ($component_edit_target->getOORequest() !== null) ) :
							?>
							
								<div class="momo_top_border">
									<div class="momo_top_spacer20 control-group">
										<label for="originatorComment" class="control-label">Request Comment</label>
										<div class="controls">
											<div id="originatorComment">
												<textarea name="originatorComment" class="input-xlarge" rows="3" disabled="disabled"><?php echo $component_edit_target->getOORequest()->getOriginatorComment(); ?></textarea>
											</div>
										</div>
									</div>
								</div>
								...
								<div class="control-group">
									<label for="requestStatus" class="control-label">Request Status</label>
									<div class="controls">
									
										<select id="requestStatus" name="requestStatus" class="span2">
										
											<?php foreach ( $component_map_allowed_request_status as $curOptVal => $curOptText ) : ?>
												
												<?php $selected = "" ?>
												<?php if (  $component_edit_target->getOORequest()->getStatus() ==  $curOptVal ) : ?>
													<?php $selected = 'selected="selected"' ?>
												<?php endif; ?>
						...
						<!-- the user that the booking applies to for -->
						<input type="hidden" name="targetUserId" value="<?php echo $component_user_target->getId(); ?>">	
